...
#
# Returns 0 (healthy) only when the board can complete a real TCP round-trip to
# at least one upstream host. A bare ICMP ping to the default gateway is NOT
# sufficient: the Pi's brcmfmac SDIO firmware keeps the association alive — and
# keeps answering at the radio layer — even when the actual datapath is wedged.
# Observed 2026-09-19 on a deployed node, which the AP reported associated at
# -49 dBm with 100% satisfaction for ~13h while the host answered nothing and
# published no telemetry. A TCP connect exercises the full TX/RX path through
# the firmware, so a wedge fails it.
#
# Targets default to the system's configured DNS servers (always-on infra the
# board already depends on, reachable only over the real datapath). The loopback
# stub (127.0.0.0/8, e.g. systemd-resolved) is excluded so it can never return a
# false "healthy" with the network down. Override or extend by setting
#   TARGETS=( "host:port" ... )
# in /etc/default/bluesnap-net-check.
#
...
[ -r /etc/default/bluesnap-net-check ] && . /etc/default/bluesnap-net-check
